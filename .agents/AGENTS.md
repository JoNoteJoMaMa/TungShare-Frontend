...
- **Pure WebTorrent P2P File Sharing**:
  - Exclusively uses WebTorrent seeding and downloading over WebSocket trackers (prioritizing custom backend `/announce` tracker first).
  ...
  - Monotonic Progress Lock: Once a download reaches 100% or `done: true`, `progress` is locked at 100% and `done: true` to prevent percentage flickering.
